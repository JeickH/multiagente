...
| 362 | **Decisión del CEO: cuánto debe durar la sesión** (heredada de #361). Hasta ahora el token vencía a los 30 min pero el frontend no se enteraba, así que la sesión "se sentía" eterna en pantallas que no llaman al backend. Ya corregido eso, la duración real se nota: hay que reingresar cada 30 min de inactividad. Se sube con la env var `ACCESS_TOKEN_EXPIRE_MINUTES` (el tope de `app/config.py` son 240 = 4 h) en la task-def de ECS y en el `.env` local | CEO + Deploy AWS | ⬜ | **No es un bug, es una decisión de producto.** Cambio de una sola env var, sin código. Si se sube, revisar que siga siendo razonable para un computador compartido. |
